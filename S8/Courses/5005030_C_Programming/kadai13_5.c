...
int chargePreca(Preca tab[], int _size){
    int id, code, amount;
    printf("Charge Process\n");
    printf("ID: ");
    scanf("%d", &id);
    printf("Security Code: ");
    scanf("%d", &code);
...
int main() {
    ...
    do{
        printf("You want to charge (0) or pay (1)? ");
        scanf("%d", &choice);
    }while (choice != 0 && choice != 1);

    if (choice == 0){
        paymentResult = chargePreca(tab, n);
    }

    if (choice == 1){
        paymentResult = payPreca(tab, n);
    }
